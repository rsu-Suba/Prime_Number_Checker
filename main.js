...
async function divide(searchnum) {
   ...
   let sqrtNum = Math.sqrt(searchnum);
   let i = 2;
   let addNum = 2;
   for (let j = 0; ; j++) {
      if (j < 1230) i = factors[j];
      else {
         i += addNum;
         addNum = addNum == 2 ? 4 : 2;
      }
      if (i % (2 || 3 || 5 || 7) == 0 && j > 25) continue;
      if (i > sqrtNum) {
         if (searchnum == setnum) {
...
async function endlessexp() {
   ...
   var datetext = `${("0" + month++).slice(-2)}-${("0" + date.getDate()).slice(-2)}-${("0" + date.getFullYear()).slice(
      -2
...
window.sleep = sleep;
window.endlessstart = endlessstart;
window.endlessdivide = endlessdivide;
window.endlessstop = endlessstop;
window.endlesslimit = endlesslimit;
window.endlessexp = endlessexp;
